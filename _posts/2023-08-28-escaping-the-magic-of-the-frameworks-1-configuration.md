...
Whether or not to use a framework is a **controversial topic**, with fairly good arguments on both sides.

**With this series of articles I want to replace, magic by magic, some framework's features until I have no more sorceries to wield**. I would also like to explain what made me want to replace them, and what really changes in the codebase.
...
I generally see the need to install an IDE plugin for a framework as a **code smell**. It means that the magic involved in the framework is too powerful for my IDE/compiler. It means that a lot of things happen at runtime, and I need to pollute my classes with `null` fields even when those fields should never be `null`.
...
```
([See this version - tag `CONFIG`](https://github.com/AlessioCoser/escaping-frameworks-magics/tree/02-CONFIG/src/main/kotlin/com/example))
...
```
([See this version - tag `SERVER_CONFIG`](https://github.com/AlessioCoser/escaping-frameworks-magics/tree/03-CONFIG-PORT/src/main/kotlin/com/example))
